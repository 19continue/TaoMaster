...
public sealed class AppLocalizer
{
    ...
    private static readonly IReadOnlyDictionary<string, string> EnglishStrings =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["doctorRepairMachinePathRecommendation"] = "A machine PATH entry is winning first. Remove or reorder it in System Properties > Environment Variables with administrator privileges.",
    ...
    private static readonly IReadOnlyDictionary<string, string> SimplifiedChineseStrings =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["busyRepairingUserPath"] = "正在清理用户 PATH 里的 JDK 与 Maven 冲突入口...",
            ...
            ["userPathAlreadyCleanStatus"] = "用户 PATH 中没有需要清理的直接 JDK 或 Maven 冲突入口。",
            ...
            ["pathScopeMachine"] = "机器级 PATH",
            ["pathScopeUser"] = "用户级 PATH",
            ["doctorNoActionNeeded"] = "无需处理。",
            ["doctorRepairUserPathRecommendation"] = "可运行 `repair user-path` 或使用桌面端按钮清理用户级 PATH 冲突入口。",
            ["doctorRepairMachinePathRecommendation"] = "当前是机器级 PATH 入口抢先命中，需要用管理员权限在“系统属性 > 环境变量”中移除或调整顺序。",
